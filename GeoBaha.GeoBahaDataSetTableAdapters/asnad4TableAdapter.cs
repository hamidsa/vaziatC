...
namespace GeoBaha.GeoBahaDataSetTableAdapters
{
	...
	public class asnad4TableAdapter : Component
	{
		...
		private void InitAdapter()
		{
			...
			dataTableMapping.DataSetTable = "asnad4";
			...
			_adapter.InsertCommand.CommandText = "INSERT INTO [dbo].[asnad4] ([sanadnum], [fopf], [Sharh], [darsad], [maghtoo], [mablagh], [mablaghk], [Salmali], [IjadSystem], [EslahSystem], [TimeIjad], [TimeEslah], [Soori], [Ghaleb]) VALUES (@sanadnum, @fopf, @Sharh, @darsad, @maghtoo, @mablagh, @mablaghk, @Salmali, @IjadSystem, @EslahSystem, @TimeIjad, @TimeEslah, @Soori, @Ghaleb)";
		...
		private void InitCommandCollection()
		{
			...
			_commandCollection[0].CommandText = "SELECT recnum, sanadnum, fopf, Sharh, darsad, maghtoo, mablagh, mablaghk, Salmali, IjadSystem, EslahSystem, TimeIjad, TimeEslah, Soori, Ghaleb FROM dbo.asnad4";
		...
		public virtual int Fill(GeoBahaDataSet.asnad4DataTable dataTable)
		{
		...
		public virtual GeoBahaDataSet.asnad4DataTable GetData()
		{
			Adapter.SelectCommand = CommandCollection[0];
			GeoBahaDataSet.asnad4DataTable asnad4DataTable = new GeoBahaDataSet.asnad4DataTable();
			Adapter.Fill(asnad4DataTable);
			return asnad4DataTable;
		}

		[DebuggerNonUserCode]
		[GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")]
		[HelpKeyword("vs.data.TableAdapter")]
		public virtual int Update(GeoBahaDataSet.asnad4DataTable dataTable)
		{
		...
		{
			return Adapter.Update(dataSet, "asnad4");
		}
		...
		public virtual int Insert(decimal sanadnum, bool fopf, string Sharh, double? darsad, double? maghtoo, double? mablagh, double? mablaghk, string Salmali, string IjadSystem, string EslahSystem, DateTime? TimeIjad, DateTime? TimeEslah, bool? Soori, bool? Ghaleb)
		{
			Adapter.InsertCommand.Parameters[0].Value = sanadnum;
			Adapter.InsertCommand.Parameters[1].Value = fopf;
			if (Sharh == null)
			{
			...
			if (darsad.HasValue)
			{
				Adapter.InsertCommand.Parameters[3].Value = darsad.Value;
			}
			else
			{
			...
			if (maghtoo.HasValue)
			{
				Adapter.InsertCommand.Parameters[4].Value = maghtoo.Value;
			}
			else
			{
			...
			if (mablagh.HasValue)
			{
				Adapter.InsertCommand.Parameters[5].Value = mablagh.Value;
			}
			else
			{
			...
			{
				Adapter.InsertCommand.Parameters[6].Value = mablaghk.Value;
			}
			else
			{
			...
			{
				Adapter.InsertCommand.Parameters[7].Value = Salmali;
			}
			if (IjadSystem == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[8].Value = IjadSystem;
			}
			if (EslahSystem == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[9].Value = EslahSystem;
			}
			if (TimeIjad.HasValue)
			{
				Adapter.InsertCommand.Parameters[10].Value = TimeIjad.Value;
			}
			else
			{
			...
			{
				Adapter.InsertCommand.Parameters[11].Value = TimeEslah.Value;
			}
			else
			{
