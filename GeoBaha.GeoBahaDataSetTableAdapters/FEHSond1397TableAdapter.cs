...
namespace GeoBaha.GeoBahaDataSetTableAdapters
{
	...
	public class FEHSond1397TableAdapter : Component
	{
		...
		[DebuggerNonUserCode]
		[GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")]
		public FEHSond1397TableAdapter()
		{
		...
		private void InitAdapter()
		{
			...
			dataTableMapping.DataSetTable = "FEHSond1397";
			...
			_adapter.InsertCommand.CommandText = "INSERT INTO [dbo].[FEHSond1397] ([Id], [radif], [shomar], [Sharh], [Fasl], [vkod], [Vahed], [baha], [darsad], [setareh], [ItemNoa], [ItemDarsad], [GroupNo], [GroupName], [Selection]) VALUES (@Id, @radif, @shomar, @Sharh, @Fasl, @vkod, @Vahed, @baha, @darsad, @setareh, @ItemNoa, @ItemDarsad, @GroupNo, @GroupName, @Selection)";
			_adapter.InsertCommand.CommandType = CommandType.Text;
			_adapter.InsertCommand.Parameters.Add(new SqlParameter("@Id", SqlDbType.Int, 0, ParameterDirection.Input, 0, 0, "Id", DataRowVersion.Current, sourceColumnNullMapping: false, null, "", "", ""));
		...
		private void InitCommandCollection()
		{
			...
			_commandCollection[0].CommandText = "SELECT Id, radif, shomar, Sharh, Fasl, vkod, Vahed, baha, darsad, setareh, ItemNoa, ItemDarsad, GroupNo, GroupName, Selection FROM dbo.FEHSond1397";
		...
		public virtual int Fill(GeoBahaDataSet.FEHSond1397DataTable dataTable)
		{
		...
		public virtual GeoBahaDataSet.FEHSond1397DataTable GetData()
		{
			Adapter.SelectCommand = CommandCollection[0];
			GeoBahaDataSet.FEHSond1397DataTable fEHSond1397DataTable = new GeoBahaDataSet.FEHSond1397DataTable();
			Adapter.Fill(fEHSond1397DataTable);
			return fEHSond1397DataTable;
		}

		[DebuggerNonUserCode]
		[GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")]
		[HelpKeyword("vs.data.TableAdapter")]
		public virtual int Update(GeoBahaDataSet.FEHSond1397DataTable dataTable)
		{
		...
		{
			return Adapter.Update(dataSet, "FEHSond1397");
		}
		...
		public virtual int Insert(int Id, string radif, string shomar, string Sharh, string Fasl, decimal? vkod, string Vahed, string baha, bool? darsad, bool? setareh, string ItemNoa, string ItemDarsad, string GroupNo, string GroupName, bool? Selection)
		{
			...
			Adapter.InsertCommand.Parameters[2].Value = shomar;
			if (Sharh == null)
			{
			...
			Adapter.InsertCommand.Parameters[4].Value = Fasl;
			if (vkod.HasValue)
			{
				Adapter.InsertCommand.Parameters[5].Value = vkod.Value;
			}
			else
			{
			...
			Adapter.InsertCommand.Parameters[6].Value = Vahed;
			if (baha == null)
			{
				throw new ArgumentNullException("baha");
			}
			Adapter.InsertCommand.Parameters[7].Value = baha;
			if (darsad.HasValue)
			{
				Adapter.InsertCommand.Parameters[8].Value = darsad.Value;
			}
			else
			{
			...
			if (setareh.HasValue)
			{
				Adapter.InsertCommand.Parameters[9].Value = setareh.Value;
			}
			else
			{
			...
			Adapter.InsertCommand.Parameters[10].Value = ItemNoa;
			if (ItemDarsad == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[11].Value = ItemDarsad;
			}
			if (GroupNo == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[12].Value = GroupNo;
			}
			if (GroupName == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[13].Value = GroupName;
			}
			if (Selection.HasValue)
			{
				Adapter.InsertCommand.Parameters[14].Value = Selection.Value;
			}
			else
			{
