...
namespace GeoBaha.GeoBahaDataSetTableAdapters
{
	...
	public class etkarfarTableAdapter : Component
	{
		...
		private void InitAdapter()
		{
			...
			dataTableMapping.DataSetTable = "etkarfar";
			...
			_adapter.InsertCommand.CommandText = "INSERT INTO [dbo].[etkarfar] ([radif], [kknum], [kname], [adres], [tel], [ShomareEghtesadi], [ShomareSabtYaMelli], [okod], [Oname], [ShahrKod], [ShahrName], [KodPosti], [IjadSystem], [EslahSystem], [TimeIjad], [TimeEslah]) VALUES (@radif, @kknum, @kname, @adres, @tel, @ShomareEghtesadi, @ShomareSabtYaMelli, @okod, @Oname, @ShahrKod, @ShahrName, @KodPosti, @IjadSystem, @EslahSystem, @TimeIjad, @TimeEslah)";
		...
		private void InitCommandCollection()
		{
			...
			_commandCollection[0].CommandText = "SELECT indentradif, radif, kknum, kname, adres, tel, ShomareEghtesadi, ShomareSabtYaMelli, okod, Oname, ShahrKod, ShahrName, KodPosti, IjadSystem, EslahSystem, TimeIjad, TimeEslah FROM dbo.etkarfar";
		...
		public virtual int Fill(GeoBahaDataSet.etkarfarDataTable dataTable)
		{
		...
		public virtual GeoBahaDataSet.etkarfarDataTable GetData()
		{
			Adapter.SelectCommand = CommandCollection[0];
			GeoBahaDataSet.etkarfarDataTable etkarfarDataTable = new GeoBahaDataSet.etkarfarDataTable();
			Adapter.Fill(etkarfarDataTable);
			return etkarfarDataTable;
		}

		[DebuggerNonUserCode]
		[GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")]
		[HelpKeyword("vs.data.TableAdapter")]
		public virtual int Update(GeoBahaDataSet.etkarfarDataTable dataTable)
		{
		...
		{
			return Adapter.Update(dataSet, "etkarfar");
		}
		...
		public virtual int Insert(string radif, decimal kknum, string kname, string adres, string tel, string ShomareEghtesadi, string ShomareSabtYaMelli, string okod, string Oname, string ShahrKod, string ShahrName, string KodPosti, string IjadSystem, string EslahSystem, DateTime? TimeIjad, DateTime? TimeEslah)
		{
			if (radif == null)
			{
			...
			Adapter.InsertCommand.Parameters[1].Value = kknum;
			if (kname == null)
			{
				throw new ArgumentNullException("kname");
			}
			Adapter.InsertCommand.Parameters[2].Value = kname;
			if (adres == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[3].Value = adres;
			}
			if (tel == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[4].Value = tel;
			}
			...
			{
				Adapter.InsertCommand.Parameters[5].Value = ShomareEghtesadi;
			}
			...
			{
				Adapter.InsertCommand.Parameters[6].Value = ShomareSabtYaMelli;
			}
			if (okod == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[7].Value = okod;
			}
			if (Oname == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[8].Value = Oname;
			}
			if (ShahrKod == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[9].Value = ShahrKod;
			}
			if (ShahrName == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[10].Value = ShahrName;
			}
			if (KodPosti == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[11].Value = KodPosti;
			}
			if (IjadSystem == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[12].Value = IjadSystem;
			}
			if (EslahSystem == null)
			{
			...
			{
				Adapter.InsertCommand.Parameters[13].Value = EslahSystem;
			}
			if (TimeIjad.HasValue)
			{
				Adapter.InsertCommand.Parameters[14].Value = TimeIjad.Value;
			}
			else
			{
			...
			{
				Adapter.InsertCommand.Parameters[15].Value = TimeEslah.Value;
			}
			else
			{
