...
namespace GeoBaha.GeoBahaDataSetTableAdapters
{
	...
	public class etshahrTableAdapter : Component
	{
		...
		private void InitAdapter()
		{
			...
			dataTableMapping.DataSetTable = "etshahr";
			...
			_adapter.InsertCommand.CommandText = "INSERT INTO [dbo].[etshahr] ([okod], [shkod], [shname], [fehrest], [okod_old], [shkod_old]) VALUES (@okod, @shkod, @shname, @fehrest, @okod_old, @shkod_old)";
		...
		private void InitCommandCollection()
		{
			...
			_commandCollection[0].CommandText = "SELECT identradif, okod, shkod, shname, fehrest, okod_old, shkod_old FROM dbo.etshahr";
		...
		public virtual int Fill(GeoBahaDataSet.etshahrDataTable dataTable)
		{
		...
		public virtual GeoBahaDataSet.etshahrDataTable GetData()
		{
			Adapter.SelectCommand = CommandCollection[0];
			GeoBahaDataSet.etshahrDataTable etshahrDataTable = new GeoBahaDataSet.etshahrDataTable();
			Adapter.Fill(etshahrDataTable);
			return etshahrDataTable;
		}

		[DebuggerNonUserCode]
		[GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")]
		[HelpKeyword("vs.data.TableAdapter")]
		public virtual int Update(GeoBahaDataSet.etshahrDataTable dataTable)
		{
		...
		{
			return Adapter.Update(dataSet, "etshahr");
		}
		...
		public virtual int Insert(decimal okod, decimal shkod, string shname, string fehrest, decimal? okod_old, decimal? shkod_old)
		{
			Adapter.InsertCommand.Parameters[0].Value = okod;
			Adapter.InsertCommand.Parameters[1].Value = shkod;
			if (shname == null)
			{
				throw new ArgumentNullException("shname");
			}
			Adapter.InsertCommand.Parameters[2].Value = shname;
			if (fehrest == null)
			{
				throw new ArgumentNullException("fehrest");
			}
			Adapter.InsertCommand.Parameters[3].Value = fehrest;
			if (okod_old.HasValue)
			{
				Adapter.InsertCommand.Parameters[4].Value = okod_old.Value;
			}
			else
			{
			...
			{
				Adapter.InsertCommand.Parameters[5].Value = shkod_old.Value;
			}
			else
			{
