	...
	public class Frm_Project : Form
	{
		...
		private void Frm_Project_Load(object sender, EventArgs e)
		{
			...
			foreach (Control control in pnl_Project.Controls)
			{
				...
				{
					control.GotFocus += (object a0, EventArgs a1) =>
					{
		...
		public void ShowProjectRecord()
		{
			...
			// بررسی اولیه برای خروج اگر DataGridView خالی است
			if (dtg_Project == null || dtg_Project.RowCount == 0)
			{
				return;
			}
			...
			try
			{
				// دریافت index با بررسی null بودن
				index = (dtg_Project.CurrentRow != null) ? dtg_Project.CurrentRow.Index : -1;

				// خروج اگر index نامعتبر است
				if (index == -1)
				{
					return;
				}

				// بررسی محدوده معتبر برای index
				if (index < 0 || index >= dtg_Project.Rows.Count)
				{
					return;
				}

				// بررسی وجود ستون‌های مورد نیاز
				if (dtg_Project.Columns.Count < 11)
				{
					return;
				}

				// دریافت مقادیر با بررسی null
				project.Id = Conversions.ToInteger(dtg_Project.Rows[index].Cells[0]?.Value ?? 0);
				project.ProjectCode = Conversions.ToInteger(dtg_Project.Rows[index].Cells[2]?.Value ?? 0);
				project.ProjectName = dtg_Project.Rows[index].Cells[3]?.Value?.ToString() ?? string.Empty;
				project.BakhshCode = dtg_Project.Rows[index].Cells[7]?.Value?.ToString() ?? string.Empty;

				// بررسی مقادیر ستون‌های 4، 5 و 6
				string cell4 = dtg_Project.Rows[index].Cells[4]?.Value?.ToString() ?? string.Empty;
				string cell5 = dtg_Project.Rows[index].Cells[5]?.Value?.ToString() ?? string.Empty;
				string cell6 = dtg_Project.Rows[index].Cells[6]?.Value?.ToString() ?? string.Empty;

				if (string.IsNullOrEmpty(cell4) || string.IsNullOrEmpty(cell5) || string.IsNullOrEmpty(cell6))
				{
				...
				{
					project.ProjectOstan = cell4;
					project.ProjectShahr = cell5;
					project.ProjectBakhsh = cell6;
				}

				// دریافت بقیه مقادیر با بررسی null
				project.ProjectAddress = dtg_Project.Rows[index].Cells[8]?.Value?.ToString() ?? string.Empty;
				project.ProjectTel = dtg_Project.Rows[index].Cells[9]?.Value?.ToString() ?? string.Empty;
				project.ProjectFehrestFile = dtg_Project.Rows[index].Cells[10]?.Value?.ToString() ?? string.Empty;
